 ...
 */
	...
	protected function _handshake() {
		...
		// Initial handshake
		$http = new HttpSocket();
		$this->_handshake = $http->get('http://localhost:8124/socket.io/1/', array('t' =>(int)microtime(true)*1000)); //$this->post(array('path' => 'socket.io/1/?t=' . (int)microtime(true)*1000, 'scheme' => $this->config['scheme'], 'port' => $this->config['port']));
